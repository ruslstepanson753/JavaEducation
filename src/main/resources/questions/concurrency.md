...
## 	Что значит «приоритет потока»?
	...
	Чтобы установить приоритет потока, используется метод класса `Thread`: `final void setPriority(int level)`. Значение `level` изменяется в пределах от `Thread.MINPRIORITY = 1` до `Thread.MAXPRIORITY = 10`. Приоритет по умолчанию - `Thread.NORMPRlORITY = 5`.
...
```
	...
	- [IBM DeveloperWorks - Выполнение задач в многопоточном режиме](https://www.ibm.com/developerworks/ru/library/l-javauniversemultithreadingtasks/)
	...
	- [duct-tape-architect.ru](http://www.duct-tape-architect.ru/?p=294#3171187JavaHotSpotJVM6)
